...
public class DataSourceConfig {
  ...
  @Primary // Очень важно, чтобы этот бин был основным кандидатом
  public DataSource atomikosDataSource() {
    log.info(
      "--- transfer-service Custom Atomikos DataSourceConfig :: Creating 'dataSource' bean ---"
    );
    ...
    if (
      ...
      String errorMsg =
        "JDBC serverName or databaseName is missing for transfer-service DataSource configuration.";
      log.error(errorMsg);
    ...
    log.info(
      "Custom AtomikosDataSourceBean for transfer-service created successfully."
    );
